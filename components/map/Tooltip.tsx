...
export function Tooltip({ data, colors }: TooltipProps) {
  ...
  return (
    <div
      className={clsx(
        "pointer-events-none absolute z-[3] w-60 max-w-[95vw] rounded-xl border border-border bg-white/95 p-4 text-xs text-text-secondary shadow-lg backdrop-blur",
        pinned ? "ring-2 ring-accent/50" : null,
      )}
      style={{
        left: position.x,
        top: position.y,
    ...
    >
      ...
      {sinceValue ? <p className="mt-1 text-text-muted">Since {sinceValue}</p> : null}
      {citationUrl ? (
        <p className="mt-2">
          <a
            href={citationUrl}
